...
if( $_REQUEST["idLabel"] >0 || $_REQUEST["idEnvironnement"] >0){
  ...
  if($_REQUEST["idLabel"] >0 && $_REQUEST["idEnvironnement"] >0){
    $sql.= " centre.id_centre_ambiancet LIKE '%".$_REQUEST["idEnvironnement"]."%' 
    and centre.id_centre_detention_label LIKE '%".$_REQUEST["idLabel"]."%' ";
    /*$sql.= " centre.id_centre_environnement in (".$_REQUEST["idEnvironnement"].") 
    and centre.id_centre_detention_label in (".$_REQUEST["idLabel"].")";*/
  }
  elseif($_REQUEST["idLabel"] >0){
    $sql.= " centre.id_centre_detention_label LIKE '%".$_REQUEST["idLabel"]."%' ";
    //$sql.= " centre.id_centre_detention_label in (".$_REQUEST["idLabel"].") ";
  }
  elseif($_REQUEST["idEnvironnement"] >0){
    $sql.= " centre.id_centre_environnement LIKE '%".$_REQUEST["idEnvironnement"]."%' ";
    //$sql.= " centre.id_centre_environnement in (".$_REQUEST["idEnvironnement"].") ";
  }
}

if($_REQUEST["idCentre"] != "" || $_SESSION["idCentre"]!=""){
