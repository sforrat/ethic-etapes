...
//Pagination
// [RPL] 18/10/2010 - pas de pagination pour les résultats de recherche d'offres ---------------------------------------
...
if($previousPage == 1 || !$previousPage){
	...
	$url_previous = get_url_nav($_REQUEST['Rub'],$params_page);
}
...
if($next == 1  || !$next){
	...
	$url_next = get_url_nav($_REQUEST['Rub'],$params_page);
}
...
if($nbPages>1){
	...
	$this -> assign ('url_last_page', get_url_nav($_REQUEST['Rub'],$params_page));
}else{
...
for($i = $startPage; $i<$endPage;$i++){
	...
	if($i == 1){
		...
		$tab["url"] = get_url_nav($_REQUEST['Rub'],$params_page);
	}
