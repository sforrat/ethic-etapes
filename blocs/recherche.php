...
$objSearch -> typeOfWordsMatching = "and"; // or où and :  si plusieurs mots saisis, on les cherche  en ET oubien en OU
...
$objSearch -> tabTablesDug["classe_decouverte"] = "classe_decouverte.id_classe_decouverte";
$objSearch -> tabTablesDug["trad_classe_decouverte"] = "trad_classe_decouverte.id__classe_decouverte";
$objSearch -> tabTablesDug["cvl"] = "cvl.id_cvl";
...
$objSearch -> tabTablesDug["seminaires"] = "seminaires.id_seminaires";
...
$objSearch -> tabTablesDug["sejours_touristiques"] = "sejours_touristiques.id_sejours_touristiques";
$objSearch -> tabTablesDug["trad_sejours_touristiques"] = "trad_sejours_touristiques.id__sejours_touristiques";
$objSearch -> tabTablesDug["stages_thematiques_groupes"] = "stages_thematiques_groupes.id_stages_thematiques_groupes";
...
function GetCOntentSejour($table,$id){
	...
	switch($table){
		...
		case "trad_cvl":
			...
			break;
		case "seminaires":
			$sql = "select presentation from trad_seminaires where id__seminaire=".$id." and id__langue=".$_SESSION["ses_langue"];
			$result = mysql_query($sql);
			return coupe_espace(strip_tags(html_entity_decode(mysql_result($result,0,"presentation"))),150);
			break;
		case "trad_seminaires":
			$sql = "select presentation from trad_seminaires where id_trad_seminaire=".$id." and id__langue=".$_SESSION["ses_langue"];
			$result = mysql_query($sql);
			return coupe_espace(strip_tags(html_entity_decode(mysql_result($result,0,"presentation"))),150);
			break;
...
function nameSejour($_rstRow){

	if($_rstRow[2] == "classe_decouverte" || $_rstRow[2] == "trad_classe_decouverte"){ // sejour		
		$Rub =_NAV_CLASSE_DECOUVERTE;	
	}elseif($_rstRow[2] == "cvl" || $_rstRow[2] == "trad_cvl"){
		$Rub =_NAV_CVL;
	}elseif($_rstRow[2] == "accueil_groupes_scolaires" || $_rstRow[2] == "trad_accueil_groupes_scolaires"){
		$Rub =_NAV_ACCUEIL_GROUPES_SCOLAIRES;
	}elseif($_rstRow[2] == "accueil_reunions" || $_rstRow[2] == "trad_accueil_reunions"){
		$Rub =_NAV_ACCEUIL_REUNIONS;
	}elseif($_rstRow[2] == "seminaires" || $_rstRow[2] == "trad_seminaires"){ 
		$Rub =_NAV_SEMINAIRES;
	}elseif($_rstRow[2] == "accueil_groupes_jeunes_adultes" || $_rstRow[2] == "trad_accueil_groupes_jeunes_adultes"){
		$Rub =_NAV_ACCUEIL_GROUPE;
	}elseif($_rstRow[2] == "sejours_touristiques" || $_rstRow[2] == "trad_sejours_touristiques"){
		$Rub =_NAV_SEJOURS_TOURISTIQUES_GROUPE;
	}elseif($_rstRow[2] == "stages_thematiques_groupes" || $_rstRow[2] == "trad_stages_thematiques_groupes"){ 
		$Rub =_NAV_STAGES_THEMATIQUES_GROUPE;
	}elseif($_rstRow[2] == "accueil_individuels_familles" || $_rstRow[2] == "trad_accueil_individuels_familles"){
		$Rub =_NAV_ACCUEIL_INDIVIDUEL;
	}elseif($_rstRow[2] == "short_breaks" || $_rstRow[2] == "trad_short_breaks"){
		$Rub =_NAV_SHORT_BREAK;
	}elseif($_rstRow[2] == "stages_thematiques_individuels" || $_rstRow[2] == "trad_stages_thematiques_individuels"){
		$Rub =_NAV_STAGES_THEMATIQUES_INDIVIDUEL;
	}
	
	
	
	return get_nom_sejour($Rub,$_rstRow[0]);
}
