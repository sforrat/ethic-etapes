...
include($path."include/inc_output_filters.inc.php");
	...
	if ($_REQUEST['login'] != '')
	{
		$sql = "SELECT password 
		FROM _user 
		where login = '".add_slashes($_REQUEST['login'])."' ";
		...
		else 
		{
			...
			if ($nb == 1)
			{
				...
				$is_send =  envoie_mail($destinataire, $message, _MAIL_CONTACT_MEMBRE, get_libLocal('lib_vos_identifiants'));			
	...
	$template->display('mdp_oublie.tpl');
