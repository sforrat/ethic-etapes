...
function menuObject(name,x,y,caption, r, parent) {
  ...
  document.write('<div id="divLine' + menuCount + '" class="caption" style="top: 0; left: 0"></div>'); 
  
  this.name = name;
  this.parent = parent
  this.ref = "divDot" + menuCount;
  this.caption = "divCap" + menuCount;
  this.line = "divLine" + menuCount;
  if(r) this.radius = r;
  ...
  this.expand = function() {
    if(this.subMenus.length > 0) {
      ...
      if((!this.parent || this.parent.state == this.parent.subMenus.length) && p) {
        if(this.parent){
            this.collapseAll(this.name);
            var diff = 120 / (this.subMenus.length-1);
        }else{
        ...
        for(var i=0;i<this.subMenus.length;i++){
          ...
          this.moving = true;
          this.subMenus[i].deg = (diff*i)+this.startAngle+this.deg-60;
          this.subMenus[i].slide(this.subMenus[i].radius * cos(degToRad(this.subMenus[i].deg)), this.subMenus[i].radius * sin(degToRad(this.subMenus[i].deg)), this.name + ".subMenus[" + i + "].showCaption();" + this.name + ".moving=false;" + this.name + ".state+=1;");
        }        
        if(this.parent){
  ...
  this.slide = function(xx,yy,func) {
    ...
    if( (Math.abs(x) < v) && (Math.abs(y) < v) ) {
      ...
      /*lx = px + dx;
      ly = py + dy;
      ln = getObjectRef(this.line);
      ln.innerHTML+='<div id="divLnk'+ dx + '" class="line" style="top:'+ly+';left:'+lx+';visibility:visible">.</div>/n';*/
      this.moveBy(dx,dy);
