...
//---------------------------------
//FFR -- Sauvegarde de l'url de départ si besoin de valider et de revenir sur la fiche
...
if ($mode=="modif" || $mode=="nouveau" || $mode=="supr" || $mode=="duplicate") {
	...
	if (empty($_REQUEST['action']))  //La modif a ete validï¿½
	{
		...
		<form method="post" action="<?echo NomFichier($_SERVER['PHP_SELF'],0)."?TableDef=$TableDef&mode=$mode&ID=$ID&id=$id_item&idObj=$idObj&target=$target&action=1&Search=$Search&Page=$Page&AscDesc=$AscDesc&ordre=$ordre&DisplayMode=$DisplayMode&DisplayMenu=$DisplayMenu&formMain_selection=$formMain_selection";?>" id="formulaire" name="formulaire" enctype="multipart/form-data">
			<?
			// LAC 24/02/2004 : on passe en champ cachï¿½ l'id de l'objet source si objet dupliquï¿½
			if( $methode == "duplicate" )
			{
			?>
		...
		if ($mode=="supr")  //Supression
		{
			...
			if ($TableDef == 3)
			{
				// on recupere le nom de l'objet ï¿½ supprimer
		...
		if ($mode=="modif" || $mode=="nouveau")
		{
			...
			//SDE 05/02/2003
			//Inclusion des donnï¿½es communes ï¿½ tous les objets de type editorial (type=3)
			if ($object_type==3)
			{
			...
			if ($object_type==3)
			{
				...
				// *** liste de sï¿½lection de la langue
				...
				//On regarde quelle langue sï¿½lectionner :
			...
			for ($div=0; $div<$nbr_div; $div++)
			{
				...
				for ($k=0; $k<$nb_cols; $k++)
				{
					...
					*/
					...
					if ($k!=0)
					{
						...
						//------------------------------------------
						// -- FFR Placement des pï¿½riode de dispo --
						...
						//------------------------------------------
						// -- FFR On inclut les Activitï¿½s de loisir
						...
						//------------------------------------------
						// -- FFR Placement du tableau Dï¿½tail de l'hï¿½bergement --
						...
						//------------------------------------------
						// -- FFR Placement du flash gï¿½olocalisation centre --
						...
						if (mysql_num_rows($RSlc)>0)
						{
							...
							if (mysql_result($RSlc,0,"libelle")!="")
							{
								...
								// -- Placement du tableau Dï¿½tail de l'hï¿½bergement --
                ...
                if($_GET["TableDef"] != 536){
                	echo $form_fieldname;
									...
									<td valign="middle"><?
									//	echo $fieldvalue;
									//Si le formulaire est ratache a l'arbo et qu'on est pas sur le formulaire de crï¿½ation d'arborescence
									if (eregi("id_"._CONST_BO_CODE_NAME."nav",$fieldname) && $TableDef!=2)
									{
										echo $lib_form_rubrique;
									}

									//Gestion des listes de tables de donnï¿½es
						...
						elseif ($fieldtype==$mysql_datatype_text )
						{
							...
							//************    IMAGE OU FICHIER
							if ($fieldlen==ereg_replace("(.*)\((.*)\)","\\2",$datatype_file)) //Fichier ï¿½ uploader
							{
								if ($fieldvalue && $mode=="modif")
								{
									...
									if ( $TestImageType == 1 )
									{
										...
										// MODIF LAC 12/11 : lien absolu vers l'affichage des images dus au liens symboliques vers l'appli centralisï¿½e
								...
								// si on est dans le menu PORTFOLIO, on offre la possibilitï¿½ de lister le contenu du rep
								// sinon, on affiche le menu dï¿½roulant du portfolio
		            ...
		            ";
							...
							elseif ($fieldlen==ereg_replace("(.*)\((.*)\)","\\2",$datatype_multikey))
							{
								...
								//Si le premier enregistrement de la table concernï¿½e est (Aucun), on ne l'affiche pas
								...
								$str_multi_filter = " order by ordre"; // si champ ordre, tri par dï¿½faut
							...
							elseif ($fieldlen==ereg_replace("(.*)\((.*)\)","\\2",$datatype_key))
							{
								...
								if (eregi("^id_",$fieldname) || eregi("^id_"._CONST_BO_TABLE_PREFIX,$fieldname) || eregi("^id_"._CONST_BO_TABLE_DATA_PREFIX,$fieldname))
								{
									...
									else
									{
										...
										// on exclu le profil root de la liste dï¿½roulante si pas root connectï¿½
										...
										$str_filter = " order by ordre"; // tri par dï¿½faut sur champ ordre

										// LAC 26/01/07 : formulaire fils ouvert en popup a partir d'un form pere
										// on filtre la requete sur la valeur pï¿½re reï¿½ue pour forcer le choix du menu deroulant pere
										...
										// ï¿½ l'aide de la langue globale du fullkit
										...
										$_REQUEST["TableDef"]== _CONST_TABLEDEF_SEJOUR_FORMULE_REUNION)
											...
											if($_SESSION['ses_profil_user'] == _PROFIL_CENTRE && $form_fieldname=="field_id_centre"){
												...
												/*elseif( ($form_fieldname == "field_id_centre_classement" || $form_fieldname == "field_id_centre_classement_1") && $_SESSION['ses_profil_user'] == _PROFIL_CENTRE){
													...
													// gï¿½nï¿½re le champ select
													echo "<select class=\"".$StyleChamps."\" ".$input_allow." id=\"".$form_fieldname."\" name=\"".$form_fieldname."\">";

													//Dï¿½but ALA 05/10/2007 Donner la possibiltï¿½ de ne pas choisir
										...
										}//Fin liste deroulante donnï¿½es standards
									}// Fin GESTION DES LISTES DE TABLES DE DONNEES EXTERNES
								}
								//Si c une clï¿½ mais pas une table de donnï¿½es externes..... (heu dans quel cas..?)
								// -------------------------------------------------
								...
								// recupere les options supplï¿½mentaires liï¿½es au champ (nouveau, modif popup id par ex)
								get_option_edit_table();
