<tr>
  <td colspan="3"><a name="sejourDate" />
  <? 
$table = "sejour_date_accessible";  
$id_table = "id__".$table;
$height = 500;
$width = 700;
$tabledef_fils = _CONST_TABLEDEF_SEJOUR_DATE_ACCESSIBLE;
...
if ($_REQUEST['mode'] != "nouveau") {	
  $strSQL = "SELECT 
          $table.id_$table as id__$table ,
  			  DATE_FORMAT($table.date,'%d-%m-%Y' ) as libelle
  			FROM
  			  $table
...
echo("<table width='100%'><tr><td valign='top' colspan='3'><a name=heb style='text-decoration:none;'><hr><b>P&eacute;riode de disponibilit&eacute; du s&eacute;jour</b> </a><br><br>ce s&eacute;jour est accessible uniquement aux dates suivantes :<br><br></td></tr>");
...
if ($_REQUEST['mode'] != "nouveau") {	
	...
	echo "<br />";
	  $action_button2 = new bo_button();
		...
		$action_button2->name = "Ajouter des dates";
    $action_button2->action = "valid_form_plus_retour('#sejourDate','".$_REQUEST["TableDef"]."')";
    $action_button2->display();

	//echo("Cr&eacute;ation d'un s&eacute;jour en cours...");
	//echo("<br>Merci de valider la cr&eacute;ation du s&eacute;jour avant de cr&eacute;er les dates de disponibilit&eacute; du s&eacute;jour associ&eacute;");
