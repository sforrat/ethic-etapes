...
//---------------------------------
//FFR -- Sauvegarde de l'url de dpart si besoin de valider et de revenir sur la fiche
...
if ($mode=="modif" || $mode=="nouveau" || $mode=="supr" || $mode=="duplicate") {
	...
	if (empty($_REQUEST['action']))  //La modif a ete valid
	{
		...
		if ($mode=="modif" || $mode=="nouveau")
		{
			...
			//SDE 05/02/2003
			//Inclusion des donnes communes  tous les objets de type editorial (type=3)
			if ($object_type==3)
			{
			...
			if ($object_type==3)
			{
				...
				// *** liste de slection de la langue
				...
				//On regarde quelle langue slectionner :
          ...
          ($fieldname == "forfait_autre_commentaire")
					...
					if ($k!=0)
					{
						...
						//------------------------------------------
						// -- FFR Placement des priode de dispo --
						...
						//------------------------------------------
						// -- FFR On inclut les Activits de loisir
						...
						//------------------------------------------
						// -- FFR Placement du tableau Dtail de l'hbergement --
						...
						//------------------------------------------
						// -- FFR Placement du flash golocalisation centre --
						...
						if (mysql_num_rows($RSlc)>0)
						{
							...
							if (mysql_result($RSlc,0,"libelle")!="")
							{
								...
								// -- Placement du tableau Dtail de l'hbergement --
                ...
                if($_GET["TableDef"] != 536){
                	echo $form_fieldname;
									...
									<td valign="middle"><?
									//	echo $fieldvalue;
									//Si le formulaire est ratache a l'arbo et qu'on est pas sur le formulaire de cration d'arborescence
									if (eregi("id_"._CONST_BO_CODE_NAME."nav",$fieldname) && $TableDef!=2)
									{
										echo $lib_form_rubrique;
									}

									//Gestion des listes de tables de donnes
							...
							if ($fieldlen==ereg_replace("(.*)\((.*)\)","\\2",$datatype_file)) //Fichier  uploader
							{
								if ($fieldvalue && $mode=="modif")
								{
									...
									if ( $TestImageType == 1 )
									{
										...
										// MODIF LAC 12/11 : lien absolu vers l'affichage des images dus au liens symboliques vers l'appli centralise
								...
								// sinon, on affiche le menu droulant du portfolio
		            ...
		            ";
							...
							elseif ($fieldlen==ereg_replace("(.*)\((.*)\)","\\2",$datatype_multikey))
							{
								...
								//Si le premier enregistrement de la table concerne est (Aucun), on ne l'affiche pas
								...
								$str_multi_filter = " order by ordre"; // si champ ordre, tri par dfaut
							...
							elseif ($fieldlen==ereg_replace("(.*)\((.*)\)","\\2",$datatype_key))
							{
								...
								if (eregi("^id_",$fieldname) || eregi("^id_"._CONST_BO_TABLE_PREFIX,$fieldname) || eregi("^id_"._CONST_BO_TABLE_DATA_PREFIX,$fieldname))
								{
									...
									else
									{
										...
										// on exclu le profil root de la liste droulante si pas root connect
										...
										$str_filter = " order by ordre"; // tri par dfaut sur champ ordre

										// LAC 26/01/07 : formulaire fils ouvert en popup a partir d'un form pere
										// on filtre la requete sur la valeur pre reue pour forcer le choix du menu deroulant pere
										...
										$_REQUEST["TableDef"]== _CONST_TABLEDEF_SEJOUR_FORMULE_REUNION)
											...
											if($_SESSION['ses_profil_user'] == _PROFIL_CENTRE && $form_fieldname=="field_id_centre"){
												...
												/*elseif( ($form_fieldname == "field_id_centre_classement" || $form_fieldname == "field_id_centre_classement_1") && $_SESSION['ses_profil_user'] == _PROFIL_CENTRE){
													...
													// gnre le champ select
													echo "<select class=\"".$StyleChamps."\" ".$input_allow." id=\"".$form_fieldname."\" name=\"".$form_fieldname."\">";

													//Dbut ALA 05/10/2007 Donner la possibilt de ne pas choisir
										...
										}//Fin liste deroulante donnes standards
									}// Fin GESTION DES LISTES DE TABLES DE DONNEES EXTERNES
								}
								//Si c une cl mais pas une table de donnes externes..... (heu dans quel cas..?)
								// -------------------------------------------------
								...
								// recupere les options supplmentaires lies au champ (nouveau, modif popup id par ex)
								get_option_edit_table();
