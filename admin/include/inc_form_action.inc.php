    ...
    &&  $_REQUEST['field_date_post_facebook'] != '')
{
    ...
    if(_CONST_APPLI_URL == 'http://localhost:8881/')
        $image_fb = 'http://www.ethic-etapes.fr/'.getFileFromBDD($_REQUEST['field_visuel_facebook_port'], 'actualite', '../');
    else
...
if (isset($_REQUEST['action']) && $_REQUEST['action']==1) {//Action valide
    ...
    for($lg = 0; $lg < $nbr_div; $lg++)
    {
        ...
        if ($mode=="nouveau") {
            ...
            if( $_GET["TableDef"]==8 && $_REQUEST["field_id__profil"] == _PROFIL_CENTRE){
                $sql_I = "Insert into portfolio_rub (portfolio_rub,id__user) values('mes_images_".$_REQUEST["field_login"]."',$id_ajout)";
                $result_I 	= mysql_query($sql_I);

                //On envoi un mail au centre :
                $message = "Bonjour,<br>Nous vous confirmons que votre compte d'accès à l'outil d'administration et de gestion des contenus du site web du réseau éthic étapes a bien été créé.<br><br>Pour y accéder, <a href='http://www.ethic-etapes.fr/admin'>cliquez ici.</a><br><br>Vos données personnelles sont :<br>Identifiant : ".$_REQUEST["field_login"]." <br>Mot de passe : ".$_REQUEST["field_password"]."<br><br>L'équipe du réseau éthic étapes se tient à votre disposition pour toute information complémentaire.<br><br>Bien cordialement,<br>";
                $path="../";
                ...
                envoie_mail($_REQUEST["field_email"],$message_mail,_MAIL_WEBMASTER,$titre);
                //----------------------------
            }
            // -----------------------------------------------------------------
            ...
            // FFR --- On remplit egalement les tarifs enfants et scolaire du sejour
            ...
            // FFR --- On remplit egalement le detail des hebergement si on est dans un ajout de centre
            if($_GET["TableDef"]==_CONST_TABLEDEF_CENTRE){
        ...
        elseif ($mode=="modif") {
            ...
            if( $_GET["TableDef"]==_CONST_TABLEDEF_ACCUEIL_GROUPE ||
                ...
                if($nb>0){
                    // FFR --- On met egalement a jour les tarifs enfants et scolaires
        ...
        if($mode=="modif" || $mode=="nouveau")
        {
            ...
            if($_REQUEST["TableDef"] == _CONST_TABLEDEF_ACTUALITE || $_REQUEST["TableDef"] == _CONST_TABLEDEF_BON_PLAN){
                ...
                if($_REQUEST["TableDef"] == _CONST_TABLEDEF_ACTUALITE){
                    $tablenameBlog = "trad_actualite";
                    $id_tablename = "id__actualite";
                }else{
                    $tablenameBlog = "trad_bon_plan";
                    $id_tablename = "id__bon_plan";
                }

                $sql_S    = "Select libelle From $tablenameBlog where $id_tablename=".$_REQUEST["ID"]." and id__langue=1";
                $result_S = mysql_query($sql_S);
                MakeCsvActu(1,'',stripslashes(mysql_result($result_S,0,"libelle")),'',$_REQUEST["ID"]);

                $sql_S    = "Select libelle From $tablenameBlog where $id_tablename=".$_REQUEST["ID"]." and id__langue=2";
                $result_S = mysql_query($sql_S);
                MakeCsvActu(2,'_us',stripslashes(mysql_result($result_S,0,"libelle")),'',$_REQUEST["ID"]);

                $sql_S    = "Select libelle From $tablenameBlog where $id_tablename=".$_REQUEST["ID"]." and id__langue=3";
                $result_S = mysql_query($sql_S);
                MakeCsvActu(3,'_de',stripslashes(mysql_result($result_S,0,"libelle")),'',$_REQUEST["ID"]);

                $sql_S    = "Select libelle From $tablenameBlog where $id_tablename=".$_REQUEST["ID"]." and id__langue=5";
                $result_S = mysql_query($sql_S);
                MakeCsvActu(5,'_es',stripslashes(mysql_result($result_S,0,"libelle")),'',$_REQUEST["ID"]);
            ...
            //======================================================================
            // RPL - 08/12/2011 :
            // update du champ date_insertion des sejours dans la table de base
            //======================================================================
            if(in_array($_REQUEST["TableDef"],$GLOBALS["_CONST_TABLEDEF_SEJOUR"])){
                $upd = "UPDATE ".$tablename." SET date_insertion = '".date("Y-m-d H:m:s")."' WHERE id_".$tablename."=".$id;
                mysql_query($upd);
                if(mysql_error())
                    die(mysql_error()."Erreur de mise a jour de la date de modification du sejour.<br />");
            }
            ...
            if($_REQUEST["TableDef"] == _CONST_TABLEDEF_ACTUALITE || $_REQUEST["TableDef"] == _CONST_TABLEDEF_BON_PLAN){
                ...
                MakeCsvActu(1,'','','',$_REQUEST["ID"]);
                MakeCsvActu(2,'_us','','',$_REQUEST["ID"]);
                MakeCsvActu(3,'_de','','',$_REQUEST["ID"]);
                MakeCsvActu(5,'_es','','',$_REQUEST["ID"]);

                echo "
        <b>Administration du blog.</b>
        <br>Les actualités vont être transférées dans le blog.
        <br><br><a href='javascript:popup(\"$url\")' class='new-window' id='LienBlog'>Cliquez ici pour continuer.</a>
      ...
      <!--<script>
        ...
        elseif ($mode=="supr") {
            ...
            if($_REQUEST["TableDef"] == _CONST_TABLEDEF_ACTUALITE || $_REQUEST["TableDef"] == _CONST_TABLEDEF_BON_PLAN){
                ...
                $sql_S    = "Select libelle From trad_actualite where id__actualite=".$_REQUEST["ID"]." and id__langue=1";
                $result_S = mysql_query($sql_S);
                MakeCsvActu(1,'',stripslashes(mysql_result($result_S,0,"libelle")),1,$_REQUEST["ID"]);

                $sql_S    = "Select libelle From trad_actualite where id__actualite=".$_REQUEST["ID"]." and id__langue=2";
                $result_S = mysql_query($sql_S);
                MakeCsvActu(2,'_us',stripslashes(mysql_result($result_S,0,"libelle")),1,$_REQUEST["ID"]);

                $sql_S    = "Select libelle From trad_actualite where id__actualite=".$_REQUEST["ID"]." and id__langue=3";
                $result_S = mysql_query($sql_S);
                MakeCsvActu(3,'_de',stripslashes(mysql_result($result_S,0,"libelle")),1,$_REQUEST["ID"]);

                $sql_S    = "Select libelle From trad_actualite where id__actualite=".$_REQUEST["ID"]." and id__langue=5";
                $result_S = mysql_query($sql_S);
                MakeCsvActu(5,'_es',stripslashes(mysql_result($result_S,0,"libelle")),1,$_REQUEST["ID"]);
            ...
            else
            {
                ...
                if($_REQUEST["TableDef"]== _CONST_TABLEDEF_CENTRE){
                    ...
                    $sql_S = "select id__user,email from _user where id_centre=".$_GET["ID"];
                    $result_S = mysql_query($sql_S);


                    $message = "Nous vous informons que votre compte d'accès à l'outil d'administration et de gestion des contenus du site web du réseau éthic étapes a été supprimé.<br><br>Pour toutes informations complémentaires, merci de contacter l'équipe du siège d'éthic étapes – info@ethic-etapes.fr<br><br>Bien sincèrement";
                    $path="../";
                    // Class smarty extended pour le projet
                    require($path."include/Smarty_local.class.php");
                    $template = new Smarty_local; // instanciation nouvel objet template enrichit par les parametres locaux
                    ...
                    $titre = "Suppression d'un compte";
                    $template->assign('urlSite',_CONST_APPLI_URL);
                    $template->assign('titre',$titre);
                    $template->assign('message',$message);
                    $message_mail= $template->fetch('gab/mail/mail.tpl');

                    envoie_mail(mysql_result($result_S,0,"email"),$message_mail,_MAIL_WEBMASTER,$titre);
                ...
                //=========================================================================
                // A chaque fois qu'on cree un libelle, on maj le fichier de trad
                //===================================================================
                genereFichiersTrad($_REQUEST["TableDef"]);
                ...
                if($_REQUEST["mode"]!="supr"){
                    ...
                    if  (confirm(\"Vous devez selectionner un centre pour que votre sejour soit valide. Voulez-vous revenir sur votre fiche ?\")) {  // Clic sur OK
                      ...
                      if  (confirm(\"Vous devez saisir au moins deux 'Les Plus du sejour' pour que votre sejour soit valide. Voulez-vous revenir sur votre fiche ?\")) {  // Clic sur OK
                        ...
                        }elseif( $_REQUEST['HS_bb']=="" && $_REQUEST['HS_dp']=="" && $_REQUEST['HS_pc']=="" && $_REQUEST['HS_rs']==""
                            && $_REQUEST['MS_bb']=="" && $_REQUEST['MS_dp']=="" && $_REQUEST['MS_pc']=="" && $_REQUEST['MS_rs']==""
                            && $_REQUEST['BS_bb']=="" && $_REQUEST['BS_dp']=="" && $_REQUEST['BS_pc']=="" && $_REQUEST['BS_rs']=="" ){
                            desactive_sejour($idRef,$tablename);
                            echo "<Script>
	                      if  (confirm(\"Vous devez saisir au moins un tarif dans le tableau de l\'annee en cours pour que votre sejour soit valide. Voulez-vous revenir sur votre fiche ?\")) {  // Clic sur OK
	                           document.location.href='$retourFormulaire';
	                      }else{ document.location.href=\"".NomFichier($HTTP_SERVER_VARS["PHP_SELF"],0)."?TableDef=".$TableDef."&DisplayMode=".$DisplayMode."&AscDesc=".$AscDesc."&ordre=".$ordre."&Search=".$Search."&idItem=".$idItem."&Page=".$Page."&DisplayMenu=".$DisplayMenu."&formMain_selection=".$formMain_selection."\";}
	                    </script>";
                        }else{
                      ...
                      if  (confirm(\"Vous devez saisir au moins deux 'Les Plus du sejour' pour que votre sejour soit valide. Voulez-vous revenir sur votre fiche ?\")) {  // Clic sur OK
                        ...
                        $sql_S = "SELECT COUNT(*) as nb FROM sejour_restauration_repas INNER JOIN $tablename ON ($tablename.id_$tablename = sejour_restauration_repas.IdSejour AND $tablename.id_centre=$idCentre)";
                        $result_S = mysql_query($sql_S);
                        $nb2 = mysql_result($result_S,0,"nb");

                        /*if($_SERVER['HTTP_X_FORWARDED_FOR'] == '82.235.17.159'){
                            die($sql_S);
                        }*/
