...
<a name="loisirsDispo" />
  ...
  if($_REQUEST["TableDef"] == _CONST_TABLEDEF_ACCUEIL_GROUPE){
    $titre = "Activit&eacute;s sp&eacute;cifiques aux enfants et scolaires"; 
  }else{
    $titre = "Activit&eacute;s de loisirs disponibles";
  }
  
  
$table = "sejour_loisirs_dispo";  
$id_table = "id__".$table;
$height = 500;
$width = 700;
$tabledef_fils = _CONST_TABLEDEF_LOISIRS_SEJOUR;
  ...
  $strSQL = "SELECT 
          trad_$table.id__$table,
  			...
  			FROM
  			  ...
  			  INNER JOIN ".mysql_result($rst,0,"cur_table_name")." ON ($table.IdSejour = ".mysql_result($rst,0,"cur_table_name").".id_".mysql_result($rst,0,"cur_table_name")." AND ".mysql_result($rst,0,"cur_table_name").".id_".mysql_result($rst,0,"cur_table_name")."=".$_GET["ID"].") 
  			WHERE
...
echo("<table width='100%'><tr><td valign='top' colspan='3'><a name=heb style='text-decoration:none;'><hr><b>$titre</b> </a><br></td></tr>");
...
if ($_REQUEST['mode'] != "nouveau") {	
	...
	echo "<br />";
	  $action_button2 = new bo_button();
		...
		$action_button2->name = "Ajouter des activit&eacute;es";
    $action_button2->action = "valid_form_plus_retour('#loisirsDispo','".$_REQUEST["TableDef"]."')";
