...
$sql = "SELECT
		...
		WHERE
			$table.date_insertion!='0000-00-00 00:00:00'";
...
while($myrow = mysql_fetch_array($result)){
	...
	if($aujourdhui>$date_fin){
		...
		//envoie_mail($myrow["email"],$message_mail,_MAIL_WEBMASTER,$titre);
		envoie_mail(_MAIL_WEBMASTER,$message_mail,_MAIL_WEBMASTER,$titre);
...
$sql = "SELECT
			...
			centre.libelle,
			centre.email,
			trad_$table.nom_seminaire as nom
		FROM
			$table
		INNER JOIN
			centre ON (centre.id_centre = $table.id_centre)
...
while($myrow = mysql_fetch_array($result)){
	...
	if($aujourdhui>$date_fin){
		...
		$message 	= str_replace("##SEJOUR##",$table,$content);
		$message 	= str_replace("##DUREE##","1 mois",$message);
		$titre    = str_replace("##SEJOUR##",$table,$titre_2);
		$titre    = str_replace("##DUREE##","1 mois",$titre);
		$template->assign('titre',$titre);
		$template->assign('message',$message);
		$message_mail= $template->fetch('gab/mail/mail.tpl');
