...
function MakeCsvActu($id_langue,$suffixe){

	include_once("../include/lib_front.inc.php");
	/*$sql_S = "truncate TABLE evo_items__item";
	$query = mysql_query($sql_S);
	$sql_S = "truncate TABLE evo_postcats";
	$query = mysql_query($sql_S);
	$sql_S = "truncate TABLE evo_items__prerendering";
	$query = mysql_query($sql_S);
	$sql_S = "truncate TABLE evo_files";
	$query = mysql_query($sql_S);
	...
	while ($row = mysql_fetch_array($query)) {
		...
		$sql  = "select
				...
				from 
					actualite
				inner join
					trad_actualite on (trad_actualite.id__langue=".$row["id__langue"]." and trad_actualite.id__actualite=actualite.id_actualite)
				WHERE 
					actualite.date_fin >=NOW() and trad_actualite.libelle != ''
				order by
					actualite.date_debut DESC";
...
//$title_formated = get_formatte_membre_url_rewrited($title);
			...
			$urltitle 	= getFileFromBDD($myrow["visuel_1"],"actualite","../");
			if($urltitle!=""){
...
$width=650;
					if($type == "image/jpeg"){
					$image = "../".$urltitle;
					$img = red_image($image,$image,650);
				}
	}
				$urltitle = "<img width='$width' style='display:block;' src='"._CONST_RELATIVE_LOGICAL_PATH.$urltitle."' />";
				
				
				
				//$urltitle = "<img style='display:block;' src='"._CONST_RELATIVE_LOGICAL_PATH.$urltitle."' />";
			...
			$BlogContent.= "\"$title\",\"$urltitle $content\",$date,$status,$locale,$title_formated,,open,1,,,3,\"$content\",0,$datedeadline,0,,,,,,,,,,\n";
