...
function MakeCsvActu($id_langue,$suffixe,$titreActu,$del,$idPost){
  ...
  if($titreActu!=""){
    ...
    $sql_S    = "select post_ID from evo_items__item where (post_urltitle='".$titreSearch."' or post_urltitle like '".$idPost."-%')";
    ...
    $titreSearch = str_replace("\"","'",$titreActu);
    $titreSearch = utf8_decode($titreSearch);
    $titreSearch = addslashes($titreSearch);
    $sql_S    = "select post_ID from evo_items__item where post_title='".$titreSearch."'";
  ...
  if($del!=1){
	...
	while ($row = mysql_fetch_array($query)) {
		...
		while ($myrow = mysql_fetch_array($result)) {
			...
			$title_formated = $myrow['id'].'-'.get_formatte_membre_url_rewrited($title_formated);
        ...
        if($buffer == '#Bandeau_them/start'){
          $stop = 1;
            ...
            while($myrow_S = mysql_fetch_array($result_S)){
              $tab[] = 'RewriteRule ^'.$myrow_S["url"].'$ index.php?theme='.$myrow_S["id__actualite_thematique"].'&L='.$myrow_S["id__langue"].' [QSA,NC,L]';
            }
