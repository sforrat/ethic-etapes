...
function get_url_nav_bo($Rub)
{
	...
	// on ajoute le Rub a la fin ssi si il n'est pas d�ja positionn� dans url_nav (redirection d'une
...
<?			$traitement_fin = "</table>";
	...
	if (mysql_num_rows($RstMessageFils)) {
		...
		for ($i=1;$i<=@mysql_num_rows($RstMessageFils);$i++) {
			...
			// affichage des menus non selectionn� comme affich� pour l'admin
			$cache=0;
			...
			if (!$cache) {
				...
				if (($Mode==0)||($Mode==4)) {
					...
					// on determine si le niveau courant a au moins 1 fils autoris� pour le user connect�
					// cela permet de determiner si il faut d�rouler l'arbo ou pas
...
//Attention : utilise la biblioth�que sql2xml de PEAR
...
function make_csv_file($strsql, $item, $delimiter=";") {
	...
	$out="";
	$change = array("\n", "\r"); // caract�res interdits dans les CSV � retirer
	...
	while ($row=mysql_fetch_assoc($result))
	{
		...
		// enl�ve les caract�res sp�ciaux HTML (&eacute;...) et surtout les sauts de ligne pour format CSV
	...
	// ecrit le fichier g�n�r� sur le disque
...
//Ecrit $contenu dans $file et supprime le(s) fichier(s) selon le nombre d'archives � garder : _CONST_ARCHIV_LIMIT
...
//Appelle la page for�ant le t�l�chargement du fichier $source
function telecharge($source){
	echo "<script>";
	echo "location.href='telecharge.php?src=".$source."';";
	echo "</script>";
}
//Supprime tout les element de la base de donn�es qui n'appartienne pas au site
...
// 29/10/2002
// retourne le numero � suffixer a un champ table annexe etrangere ajoutee dans une table
...
function list_dir($chdir,$id_item_parent1,$mon_dir="")
{
	...
	if(is_array($sdirs))
	{
		...
		{
			$id_item++;
			// on n'affiche pas les r�pertoires
...
// D�tection windows + IE : renvoie true si d�t�ct�
function isWIE()
{
