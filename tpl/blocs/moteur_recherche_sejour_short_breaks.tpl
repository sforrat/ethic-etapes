					<dl id="filter_results">
						...
						<dd class="clear">
							...
							<input type="hidden" name="requestFilter" value="1"/>
								...
								<fieldset>							
									<label for="short_break_theme_filter">#{smarty_trad value='lib_theme_sejour'}# :</label>
									<select id="short_break_theme_filter" name="short_break_theme_filter">
										<option value="">- #{smarty_trad value='lib_indifferent'}# -</option>
										#{section name=the loop=$listeThemeShortBreak}#
										<option value="#{$listeThemeShortBreak[the].id}#" #{if $listeThemeShortBreak[the].current}# selected="selected" #{/if}#>#{$listeThemeShortBreak[the].libelle}#</option>
										#{/section}#
									</select>
								</fieldset>								
