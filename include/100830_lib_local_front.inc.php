...
function getActualites(&$listeActu, $_params = "")
{
	...
	for ($i = 0 ; $i < $nbActu ; $i++)
	{
		...
		$lien = get_url_nav(_NAV_ACTUALITE,$params);
...
function getListeCentreEnvironnement(&$listeEnvironnement)
{
	...
	return $nbCEnv;
} // getCentreEnvironnement(&$listeEnvironnement)
...
function getSejourRand()
{
	...
	//On r�cup�re un s�jour au hasard
...
{
	//INFOS : Nom centre - Ville - Pr�sentation centre - Image accueil scolaire
...
{
	// INFOS : VILLE - Nom Centre - Texte Centre - Image s�jour
	$arrayChampSelect = array();
...
function getListeSejourSeminaires(&$listeRes, $_params = "")
{
	...
	if (isset($_params['id_sejour']))
	{
		...
		"nb_chambre", "nb_couvert", "nb_lit","capacite_salle_min","capacite_salle_max", "id_centre_service", "id_centre_activite", "nb_salle_reunion",
		"id_centre_detention_label"); //Onglet Lieu du s�jour
		foreach ($arrayChampSelect as $champ)
		$champSelect .= " ,".$champ;
	}
...
function getListeSejourAccueilGroupe(&$listeRes, $_params = "")
{
	...
	if (isset($_params['id_sejour']))
	{
		...
		"acces_route_4", "acces_train_4", "acces_avion_4", "acces_bus_metro_4", "latitude", "longitude", //Onglet acc�s
		"id_centre_activite", "id_sejour_services_sportifs", "sports_adaptes_FFH_4", "sports_adaptes_FFH_libelle",//Onglet Les activit�s
		"sejour_preparation", "sejour_preparation_commentaire", "sejour_rentree", "sejour_rentree_commentaire", "sejour_oxygenation", "sejour_oxygenation_commentaire", "forfait_autre", "forfait_autre_commentaire",
		"id_centre_detention_label");
		foreach ($arrayChampSelect as $champ)
		$champSelect .= " ,".$champ;
	}
	...
	if ($nbTotal > 0)
	{
		...
		for ($i = 0 ; $i < $nbRes ; $i++)
		{
			...
			if (isset($_params['id_sejour']))
			{
				...
				{
					$listeRes [$i]["commentaire_accueil_sportifs"] = getTradTable("accueil_groupes_jeunes_adultes",$_SESSION['ses_langue'],"commentaire_accueil_sportifs", $id);
					//$listeRes [$i]["sports_adaptes_FFH_libelle"] = getTradTable("accueil_groupes_jeunes_adultes",$_SESSION['ses_langue'],"sports_adaptes_FFH_libelle", $id);
				}
...
function getSejourSalleReunion ($Rub, $idSejour, &$listeSalle)
{
	...
	for ($i = 0 ; $i < $nbRes ; $i++)
	{
		...
		"tableau_blanc" => mysql_result($rst, $i, 'tableau_blanc_28'),
		"sonorisation" => mysql_result($rst, $i, 'sonorisation_28'),
		"paperboard" => mysql_result($rst, $i, 'paperboard_28'),
		"ecran" => mysql_result($rst, $i, 'ecran_28'),
		"climatisation" => mysql_result($rst, $i, 'climatisation_28'),
		"wifi_adsl" => mysql_result($rst, $i, 'wifi_adsl_28'));
	}
...
function getListeCentreAvisInternaute(&$listeAvis, $_params = "")
{
	$sql = "SELECT
				...
				laissez_avis.nom, 
				laissez_avis.prenom, 
				DATE_FORMAT(laissez_avis.date_auto,'%m/%d/%Y') as date, 
				laissez_avis.email, 
