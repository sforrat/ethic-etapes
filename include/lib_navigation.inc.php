...
function get_menu_gauche( $Rub, $navID, $db, $typeNav )
{
	...
	$strFreres = "SELECT
            ...
            AND
                FIND_IN_SET('".$_SESSION["ses_langue"]."',n.id__langue)>0 ";			

	if($typeNav == _NAV_FOOTER){
		$strFreres .=" and id__nav_pere="._NAV_ACCUEIL;
		// RPL 31/05/2011 : espace membre actif FR uniquement
		if($_SESSION['ses_langue'] != 1)
			$strFreres .= " AND n.id__nav <> "._NAV_ESPACE_MEMBRE;
	}
		...
		if($typeNav != _NAV_FOOTER){
			...
			while($row2 = $db->sql_fetchrow($RS2))
			{
				...
				if($row2[id__nav] == _NAV_CENTRES_NEW){
					$sql = "select count(*)as nb from centre where DATE_ADD(date_inscription, INTERVAL 6 MONTH) >= NOW() and etat=1 and FIND_IN_SET('".$_SESSION["ses_langue"]."',id__langue)>0";
