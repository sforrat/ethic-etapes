...
function getListeCentre (&$listeCentre, $_params = "")
{
    ...
    //**Reunions
    if (isset($_params['reunion'])){
        $liste ="";
        $sql_reu = "SELECT id_centre FROM accueil_reunions WHERE etat = 1";
        $rst_reu = mysql_query($sql_reu);
        $cmpt = 0;
        while($row = mysql_fetch_array($rst_reu, MYSQL_ASSOC)){
            if($cmpt >0){
               $liste .=','; 
        ...
        $where .= (($where == "") ? '' : " AND ")." c.id_centre IN ($liste)";
    }
    ...
    //echo "<!--SESSION['last_rub']=".$_SESSION['last_rub']."-->";
	return $nbTotal;
...
{
    $rand = $_SESSION['rand'];
    if (empty($rand) || ($_SESSION["last_rub"] != $_REQUEST["Rub"] && $_COOKIE["last_rub"] != $_REQUEST["Rub"] )  ) {
    ...
    setcookie("last_rub",$_REQUEST["Rub"],time()+3600*24);
...
function getSejourReunionInfos ($Rub, $idSejour, $type, &$listeInfo)
{
	...
	$listeInfo = array();
	$sql = "SELECT id_sejour_".$type.", tarif FROM sejour_".$type." WHERE IdSejour =".$idSejour." AND (id__table_def = ".$GLOBALS["_NAV_SEJOUR_TABLEDEF"][$Rub]." or id__table_def=0) ";
...
function getListeCentreTarifs(&$listeTarifs, $aQueryData, $_params = "")
{
	...
	while($row = mysql_fetch_object($rst))
	{
		$listeTarifs[$row->id_centre][] = array(		
		"bb" => $row->HS_bb, // Haute saison B & B
		...
		"T_MS_plus" => $row->moyenne_saison_n1,
		"libelle" => mb_strtoupper(get_libLocal('lib_moyenne_saison')));

		$listeTarifs[$row->id_centre][] = array(		
		"bb" => $row->BS_bb, // Haute saison B & B
