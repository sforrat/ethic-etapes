...
switch($_POST["contact_youAre"]){
	...
	case 3:
		...
		break;	
	case 4:
		$youAre = get_libLocal('lib_to_scolaire');
		break;	
	case 5:
		$youAre = get_libLocal('lib_to_groupe');
		break;	
	case 6:
...
$titre = get_libLocal('lib_titre_mail_new_centre');
$template->assign('urlSite',_CONST_APPLI_URL);
$template->assign('titre',$titre);
$template->assign('message',$message);
$message_mail= $template->fetch('gab/mail/mail.tpl');
			
envoie_mail(_MAIL_CONTACT_EMPLOI,$message_mail,$_POST["test-email-contact_mail"],get_libLocal('lib_titre_mail_new_centre'));
$param[0]["id"] = "ok";
$param[0]["id_name"] = "envoi";

redirect($path.get_url_nav(_NAV_FORMULAIRE_NEW_CENTRE,$param));
?>